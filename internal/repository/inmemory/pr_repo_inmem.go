...
var _ repo.PullRequestRepository = (*PRRepoInMemory)(nil)
...
func (r *PRRepoInMemory) Create(pr *domain.PullRequest) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *PRRepoInMemory) ListByReviewer(userID string) ([]*domain.PullRequest, error) {
	...
	for _, pr := range r.prs {
		if (pr.Reviewer1 != nil && *pr.Reviewer1 == userID) || (pr.Reviewer2 != nil && *pr.Reviewer2 == userID) {
			res = append(res, pr)
		}
	}
	return res, nil
